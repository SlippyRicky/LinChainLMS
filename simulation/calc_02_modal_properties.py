...
Outputs:
    ...
    - data/dos_asymmetry_N4000.npz
    - data/matrix_heatmap_N12.npz
...
def compute_modes(N, eps, is_diatomic=False):
    ...
    Args:
        ...
        eps (float): Disorder strength.
        is_diatomic (bool): If True, alternates masses (1.0, 3.0) before applying disorder.
    """
    t0 = time.time()
...
# def compute_heatmap():
#     """
#     Generates a small (N=12) dynamical matrix for both a pristine and a disordered chain.
#     Used for creating a visual matrix heatmap to illustrate off-diagonal and diagonal
#     disorder in the report.
#     """
#     t0 = time.time()
#     print("\n[Modal Analysis] Computing Matrix Heatmap (N=12)...")
#     N = 12
#     np.random.seed(42)
#     epsilon = 1.0
    
#     K_perf = np.ones(N-1)
#     # Rebuild full matrix since heatmap visualizes the full matrix
#     D_perf = np.zeros((N, N))
#     for i in range(N):
#         kl = K_perf[i-1] if i > 0 else 0
#         kr = K_perf[i] if i < N-1 else 0
#         D_perf[i, i] = (kl + kr) / 1.0
#         if i < N-1:
#             off = -K_perf[i] / 1.0
#             D_perf[i, i+1] = off
#             D_perf[i+1, i] = off
            
#     K_dis = np.random.uniform(1 - 0.5*epsilon, 1 + 0.5*epsilon, N-1)
#     D_dis = np.zeros((N, N))
#     for i in range(N):
#         kl = K_dis[i-1] if i > 0 else 0
#         kr = K_dis[i] if i < N-1 else 0
...
if __name__ == '__main__':
    print("=================================================================")
    print("   RUNNING MODAL PROPERTIES (CALC 02)                            ")
    print("=================================================================")
    ...
    compute_modes(2000, 0.4, is_diatomic=True)
    compute_dos_asymmetry()
    # compute_heatmap()
    compute_diatomic_modes_csv()
    print("\n=================================================================")
    print("   CALC 02 COMPLETE!                                             ")
    print("=================================================================")
