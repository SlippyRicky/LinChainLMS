...
def compute_mass_audit():
    """
    Performs a high-fidelity Monte Carlo audit of mass disorder distributions.
    ...
    params = sim_config.get_params("sim_01")
    N = params['N']
    M = params['M']
    eps_vals = np.linspace(0.01, 1.2, params['n_eps'])
    omega_vals = np.linspace(0.1, 2.1, params['n_omega'])
    K = 1.0
    m0 = 1.0
    ...
    for i, eps in enumerate(eps_vals):
        ...
        if (i + 1) % max(1, total_steps // 5) == 0 or i == total_steps - 1:
...
def compute_stiffness_audit():
    """
    Performs a high-fidelity Monte Carlo audit of stiffness disorder distributions.
    Computes the spatial decay constant (gamma) across various frequencies and
    disorder strengths (epsilon) to verify the statistical properties of the generator.
    """
...
if __name__ == "__main__":
    print("=================================================================")
    print("   RUNNING DIAGNOSTICS & BASELINES (SIM 01)                      ")
    print("=================================================================")
    compute_mass_audit()
    compute_stiffness_audit()
    print("\n=================================================================")
    print("   SIM 01 COMPLETE!                                              ")
    print("=================================================================")
