...
chains and Velocity Verlet integration for large thermodynamic chains.
...
def compute_comparative_dynamics_verlet():
    """
    Simulates the time evolution of a transient wavepacket on a large chain
    using Velocity Verlet integration for 3 different disorder levels.
    """
    params = sim_config.get_params("sim_04")
    N = params['N_verlet']
    T_max = params['T_max']
    downsample = params['downsample']
    
    print(f"\n[Wavepacket Dynamics] Computing Comparative Dynamics via Velocity Verlet (N={N})...")
    t0 = time.time()
