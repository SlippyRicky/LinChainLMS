...
def compute_wp_attenuation():
    ...
    and combined disorder over a sweep of carrier frequencies.
    Uses exact diagonalization on N atoms.
    """
    print("=================================================================")
    print("   RUNNING WAVEPACKET ATTENUATION (SIM 05)                       ")
    print("=================================================================")
    t0 = time.time()
    
    params = sim_config.get_params("sim_05")
    N = params['N_verlet']
    omega_vals = np.linspace(0.1, 1.8, params['n_freqs'])
    ...
    for sys_name in ["mass", "spring", "combined"]:
        ...
        for i, w in enumerate(omega_vals):
            ...
            if (i+1) % max(1, len(omega_vals) // 3) == 0 or i == len(omega_vals) - 1:
